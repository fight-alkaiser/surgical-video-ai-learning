...
# ----------------------------------------
# Day44's autoregressive decoder failed badly (4-5x worse than
# Day42's baselines) because it was trained with pure teacher forcing
# (always fed the TRUE previous delta) but evaluated free-running
# (fed its OWN previous prediction) -- a severe train/eval mismatch
# (exposure bias). The example plots showed the free-running rollout
# converging to a generic, input-independent curve once its own
# small errors pushed it off the distribution of deltas it was
# trained on.
#
# Today's fix targets that mismatch directly, without abandoning
# autoregression: scheduled sampling (Bengio et al., 2015). During
# training, at each decode step, the model is fed its OWN previous
# prediction instead of the true delta with probability p, where p
# ramps from 0 (pure teacher forcing, like Day44's epoch 1) to 1.0
# (pure free-running, exactly matching evaluation) linearly over
# training. By the final epoch, training conditions are identical to
# evaluation conditions -- the model has no choice but to learn to
# tolerate and recover from its own errors, because that's the only
# regime it's trained in by the end.
#
# Same architecture as Day44 (GRU encoder -> GRUCell decoder,
# per-step displacement), same held-out subject, same baselines, same
# two metrics (displacement error + frame-to-frame step size).
# ----------------------------------------
...
def build_arrays(windows):
    ...
    for name, t in windows:
        ...
        y = signal[t - 1:t + OUTPUT_FRAMES, 0:3]
        inputs.append(x)
        targets.append(y)
    return np.stack(inputs).astype(np.float32), np.stack(targets).astype(np.float32)


train_x, train_y_pos = build_arrays(train_windows)
test_x, test_y_pos = build_arrays(test_windows)

train_y_delta = np.diff(train_y_pos, axis=1)
test_y_delta = np.diff(test_y_pos, axis=1)

# ----------------------------------------
# Model: identical to Day44, plus a `sampling_prob` argument that
# controls how often each step uses its own prediction instead of
# the true delta during training.
# ----------------------------------------
...
class AutoregressiveTrajectoryGRU(nn.Module):
    ...
    def forward(self, x, teacher_deltas=None, sampling_prob=0.0):
        ...
        for step in range(self.output_frames):
            ...
            if teacher_deltas is None:
                prev_delta = delta_pred
            elif sampling_prob <= 0:
                prev_delta = teacher_deltas[:, step, :]
            else:
                true_delta = teacher_deltas[:, step, :]
                use_own = (torch.rand(batch_size, 1, device=x.device) < sampling_prob).float()
                prev_delta = use_own * delta_pred.detach() + (1 - use_own) * true_delta
...
print("\n=== Training with scheduled sampling (linear ramp 0 -> 1.0) ===")
for epoch in range(NUM_EPOCHS):
    sampling_prob = epoch / max(1, NUM_EPOCHS - 1)  # 0.0 at epoch 0, 1.0 at final epoch
    model.train()
    ...
    for start in range(0, num_train, BATCH_SIZE):
        ...
        optimizer.zero_grad()
        pred_deltas = model(batch_x, teacher_deltas=batch_y_delta, sampling_prob=sampling_prob)
    ...
    if (epoch + 1) % 5 == 0 or epoch == 0:
        print(f"Epoch {epoch + 1}/{NUM_EPOCHS}: sampling_prob={sampling_prob:.2f}, "
              f"train MSE = {epoch_loss / num_batches:.8f}")

# ----------------------------------------
# Evaluate: free-running rollout, identical protocol to Day44, same
# baselines recomputed on the same held-out windows.
# ----------------------------------------
...
test_last_pos = test_y_pos[:, 0, :]
sched_pred_xyz = test_last_pos[:, None, :] + np.cumsum(pred_deltas, axis=1)
test_true_xyz = test_y_pos[:, 1:, :]
...
methods = {
    "scheduled_sampling_gru": sched_pred_xyz,
...
torch.save(model.state_dict(), output_dir / "scheduled_sampling_model.pt")
