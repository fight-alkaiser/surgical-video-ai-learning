...
model.load_state_dict(torch.load(script_dir / "scheduled_sampling_model.pt", map_location="cpu"))
model.eval()
...
for i, trial_name in enumerate(test_trials[:4]):
    ...
    sched_pred_xyz = last_pos + np.cumsum(pred_deltas, axis=0)

    ax = fig.add_subplot(2, 2, i + 1, projection="3d")
    input_mm = input_raw[:, 0:3] * 1000
    true_mm = true_xyz * 1000
    sched_mm = sched_pred_xyz * 1000
    ...
    ax.plot(*np.vstack([input_mm[-1], sched_mm]).T, color="tab:red",
            linewidth=1.8, linestyle="-.", label="pred: scheduled-sampling GRU")
...
plt.suptitle(
    "Day45: scheduled-sampling GRU, free-running rollout -- prediction uses only frames "
    "before t (red dot); black = actual future, red = model's own free-running prediction",
    fontsize=10,
)
plt.tight_layout()
