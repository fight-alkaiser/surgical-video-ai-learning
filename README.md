...
Day 46 - Smoothness-Regularized Single-Shot Model: Real Progress, Still Not Solved

* Went back to Day43's accurate-but-jagged single-shot architecture and added a jerk penalty to the loss (swept over lambda = 0/1/10/50), avoiding Day44-45's autoregression and exposure-bias risk entirely. Step size dropped 59% (1.258mm to 0.512mm, close to ground truth's 0.388mm) with essentially no accuracy cost (4.14mm to 4.19mm, still clearly ahead of both baselines) — the arc's best combined accuracy-smoothness result.
* A third diagnostic (path efficiency: net displacement over total path length) showed this wasn't the full picture — even the best-regularized model's paths are less than half as directionally efficient as real motion (0.285 vs. 0.705), still oscillating rather than progressing purposefully. This prompted a broader discussion: forecasting a human-operated instrument's future path from motion history alone may be trying to predict something genuinely under-determined by that input, since the actual next move depends on the surgeon's moment-to-moment judgment. Closes the unconditional trajectory-forecasting line (Day42-46); Day47 pivots to gesture-conditioned motion modeling (model the dynamics of a *known* gesture using JIGSAWS' existing labels, rather than guess an undetermined future choice), with anomaly/deviation detection (tied to Day40's hazard-awareness framing) as a further direction after that.

See [day46 details](day46_smoothness_regularized_model/README.md).

Day 47 - Gesture-Conditioned Motion Modeling: A Weak, Mixed Result

* Tested Day46's pivot directly: does conditioning on the true gesture label (oracle, following the CholecT50 series' Day37 precedent) improve trajectory prediction, especially path efficiency? Restricted to windows where a single gesture covers the full 60-frame span, both an unconditioned and a gesture-conditioned model (Day46's recipe plus a gesture one-hot concatenated to the final hidden state) were trained and evaluated identically.
* Gesture conditioning made accuracy and smoothness worse (3.91mm to 4.27mm; 0.770mm to 1.013mm step size) for a small path-efficiency gain (0.151 to 0.211, still far from ground truth's 0.691) — visually confirmed as more erratic, not less. A follow-up check of the owner's sharper hypothesis (needle-driving through tissue, G3, should be mechanically constrained and thus far more predictable than other gestures) found no standout signal for G3 specifically — likely because the tracked signal is the instrument driver's position, not the needle tip's, and whole-gesture windows dilute any purer in-tissue-arc signal. A genuine, multi-angle null result for gesture-level conditioning as tested.

See [day47 details](day47_gesture_conditioned_model/README.md).

Day 48 - Retrospective: Closing the Trajectory-Forecasting Arc (Day42-47)

* Closes six days of trajectory forecasting on JIGSAWS' Suturing task: naive baselines, a single-shot GRU, autoregressive decoding, scheduled sampling, smoothness regularization, and gesture conditioning. Six structurally different methods converge on the same result — no approach achieves both real accuracy (beating simple baselines) and genuine physical plausibility (smoothness and directed motion) at once, with the single-shot model refined by a jerk penalty (Day46) coming closest but still less than half as directionally efficient as real motion.
* Five cross-cutting lessons (a model can win on one metric while being obviously wrong, requiring three independent diagnostics to get a complete picture; a named risk can materialize at full severity; fixing one failure mode can cost what was already working; a well-motivated reframing can still fail and that's real information; convergent evidence across unrelated methods points at the task, not the models). Concludes this is closer to an information limit than an unsolved modeling problem. The project moves next to anomaly/deviation detection (is current motion typical for its context), with skill assessment held as a later direction.

See [day48 details](day48_trajectory_forecasting_retrospective/README.md).

Day 49 - Gesture-Conditioned Deviation Detection: Confounded by Speed, Not Skill

* Built a simple, interpretable deviation score (per-gesture z-score sum over velocity/rotation/gripper features, no neural network) defining "normal" motion from a reference group of trials, then checked whether it correlates with skill (self-proclaimed N/I/E and GRS) that was never used to fit it. First reference group (self-proclaimed experts) gave a confusing result; diagnosis found self-proclaimed skill doesn't track GRS in this dataset at all (self-proclaimed "experts" average the lowest GRS of the three groups). Switching to a top-GRS reference group shrank but didn't fix the problem.
* Root cause, confirmed directly: tooltip speed itself correlates positively with GRS in this dataset (r=+0.62) — skilled work here is faster, not more "average" — so a deviation score built from raw velocity mechanically produces a positive (wrong-direction) correlation with GRS regardless of reference group. A precise, actionable diagnosis rather than an ambiguous one: the fix isn't abandoning deviation detection, it's rebuilding it on movement-economy features (jerk, path efficiency — both already built in Day46) to test the more literature-consistent hypothesis that skill shows up in *how* motion is executed, not how fast.

See [day49 details](day49_gesture_conditioned_anomaly_detection/README.md).
