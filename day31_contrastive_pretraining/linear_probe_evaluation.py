...
# ----------------------------------------
# Evaluate the contrastively-pretrained backbone
# (contrastive_pretraining.py) the same way every instrument
# day has been evaluated: freeze the backbone completely, cache
# its 512-d features (Day24's shortcut), and train a linear
# probe with instrument LABELS -- used here for the first time
# in this backbone's existence, only at evaluation time, never
# during pretraining.
#
...
#   Day31 (ImageNet + label-free contrastive layer4 adaptation): ?
# ----------------------------------------
...
# extraction logic to Day21/26/27), and the same video-level
# split (same seed, so identical train/test videos to Day31's
# pretraining script and every earlier instrument day).
# ----------------------------------------
...
# ----------------------------------------
# Load the contrastively-pretrained, now-frozen backbone.
# ----------------------------------------
...
state_dict_path = Path(__file__).parent / "contrastive_backbone.pt"
...
print(f"\nLoaded contrastively-pretrained backbone from {state_dict_path}")

# ----------------------------------------
# Cache features (same shortcut as Day24, now on the
# contrastively-adapted backbone).
# ----------------------------------------
...
# ----------------------------------------
# Class-weighted linear probe (same recipe as Day26, so the
# comparison to Day27 isolates backbone quality, not loss
# design).
# ----------------------------------------
...
for i, name in enumerate(instrument_names):
    ...
    print(f"{name:12s} {f1:8.3f} {precision:10.3f} {recall:8.3f} "
          f"{day21_f1_reference[name]:10.3f} {day27_f1_reference[name]:10.3f}")
